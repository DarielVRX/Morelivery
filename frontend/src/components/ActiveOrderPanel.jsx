...
export default function ActiveOrderPanel({
  ...
  const canRelease  = !['on_the_way','delivered','cancelled'].includes(order.status) && !order.is_disputed;
  const canCancelDispute = order.is_disputed === true;
      ...
      <div style={{ fontSize: '0.82rem', marginTop: '0.15rem', overflow: 'hidden',
        ...
        <div style={{ fontSize: '0.72rem', color: 'var(--text-secondary)', marginTop: '0.1rem',
          ...
          opacity: !canDeliver ? 0.5 : 1,
                ...
                {/* Botón Cancelar disputa (visible solo cuando is_disputed = true) */}
                {canCancelDispute && (
                  <button
                  style={{
                    width: '100%', padding: '0.55rem 0', borderRadius: 8, fontWeight: 700,
                    fontSize: '0.82rem', border: '1.5px solid #f59e0b', cursor: 'pointer',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                    background: '#fffbeb', color: '#b45309',
                  }}
                  onClick={onCancelDispute}
                  disabled={loadingStatus === 'cancel_dispute'}>
                  <IconCancelDispute /> Cancelar disputa
                  </button>
                )}
                ...
                {order.is_disputed && !canCancelDispute && (
