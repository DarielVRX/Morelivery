// frontend/src/features/admin/dashboard/sections/DashboardTabsBar.jsx
...
export function AssignmentTab({ liveData, tick }) {
  ...
  return (
    ...
    {[
      ...
      { label:'Con oferta', value:liveData.orders.filter((order) => order.pending_driver_id && !order.driver_id).length, color:'#f59e0b' },
          { label:'Drivers disponibles', value:liveData.drivers.filter((driver) => driver.is_available).length, color:'#16a34a' },
          { label:'Drivers en entrega', value:liveData.drivers.filter((driver) => driver.active_orders > 0).length, color:'#8b5cf6' },
    ...
    {liveData.drivers.length === 0 ? (
      ...
      {sortedDrivers.map((driver) => {
        ...
        return (
          ...
          <Td><span style={{ fontWeight:600 }}>{driver.full_name?.split('_')[0] || '—'}</span></Td>
          <Td>
          {driver.is_available
            ...
            {driver.active_orders > 0
              ...
              {(driver.last_lat && driver.last_lng)
                ? <span style={{ color:'var(--success)', fontSize:'0.75rem', fontWeight:600 }}>✓ {Number(driver.last_lat).toFixed(3)},{Number(driver.last_lng).toFixed(3)}</span>
                ...
                {cooldowns.length === 0
                  ...
                  : (
                    ...
                    {cooldowns.map((cooldown, index) => (
                      ...
                      <span style={{ fontSize:'0.68rem', color:'var(--text-secondary)' }}>{cooldown.order_id.slice(0,6)}</span>
