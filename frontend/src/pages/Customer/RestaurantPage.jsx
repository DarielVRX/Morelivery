...
// ── Address search bar with MapLibre map pick ─────────────────────────────────
function AddressSearchBar({ active, onSelect }) {
  const [open,       setOpen]       = useState(false);
  const [showMap,    setShowMap]    = useState(false);
  const [pinPlaced,  setPinPlaced]  = useState(false); // drives re-render for confirm button
  ...
  // Close on outside click (not when map is open — map clicks would close it)
  useEffect(() => {
    function handler(e) {
  ...
  // Init MapLibre — runs when showMap becomes true, cleans up when false
  useEffect(() => {
    if (!showMap) return;
    let map = null;
    let cancelled = false;
                ...
                {results.length > 0 && !showMap && (
                  <div style={{ position:'absolute', top:'calc(100% + 2px)', left:0, right:0,
                    background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:8,
                                                    ...
                                                    {results.map((item, i) => (
                                                      <button key={i}
                                                      onClick={() => { onSelect(item); closeAll(); }}
                ...
                {showMap && (
                  <div style={{ position:'absolute', top:'calc(100% + 4px)', left:0, right:0,
                    background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:8,
                             ...
                             <div style={{ display:'flex', gap:'0.4rem', padding:'0.4rem 0.5rem', background:'var(--bg-sunken)' }}>
                             <span style={{ flex:1, fontSize:'0.72rem', color:'var(--text-tertiary)', alignSelf:'center' }}>
                             {pinPlaced ? '📍 Pin colocado — confirma o muévelo' : 'Toca el mapa para colocar el pin'}
                             </span>
                             <button onClick={confirmMapPin} disabled={!pinPlaced}
                             className="btn-primary btn-sm"
                             style={{ opacity: pinPlaced ? 1 : 0.45 }}>
                             Confirmar
                             </button>
