...
export default function DriverMap({
  ...
  useEffect(() => {
    ...
    if (STADIA_KEY && map) {
      // Native style swap — layers are wiped on setStyle, need to re-add after
      const newStyle = isDark ? STYLE_DARK : STYLE_LIGHT;
      if (map.isStyleLoaded()) {
        map.setStyle(newStyle);
        // Re-add route line after style loads (all custom sources/layers are cleared)
        map.once('styledata', () => {
          // Re-add route if we have geometry
          // The routeGeometry effect uses 'styledata' guard too — force a re-trigger
          // by briefly nulling and restoring (done via parent state in DriverHome)
          // Simplest: directly call draw inline
  ...
  useEffect(() => {
    ...
    ensureMapLibreJS().then((ml) => {
      ...
      map.once('load', () => {
        ...
        // Apply dark mode treatment when map loads
        const isDarkNow = document.documentElement.getAttribute('data-theme') === 'dark';
        if (isDarkNow && !STADIA_KEY) {
          // Fallback filter when no Stadia key
          const canvas = containerRef.current?.querySelector('canvas');
          if (canvas) canvas.style.filter = 'invert(1) hue-rotate(180deg) saturate(0.85) brightness(0.9)';
        }
        // With Stadia: dark style was already set at init, nothing to do
      });
      mapRef.current = map;
      onMapReady?.(map);
