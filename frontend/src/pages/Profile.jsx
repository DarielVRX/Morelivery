...
// Componente para sección Driver (capacidad de mochila)
function DriverSection({ token, bagCapacity, onBagCapacityChange }) {
  ...
  return (
    ...
    {editing ? (
      ...
      max="200"
      style={{
        width: '100px', padding: '0.4rem', borderRadius: 8,
      ...
      <button
      onClick={() => { setEditing(false); setLiters(bagCapacity); setError(''); }}
      style={{
        padding: '0.4rem 0.8rem', borderRadius: 8,
      ...
      {error && (
        <div style={{ fontSize: '0.7rem', color: '#dc2626', marginTop: '0.5rem' }}>
        {error}
        </div>
      )}
      </div>
...
export default function ProfilePage() {
  ...
  // Cargar capacidad de mochila si es driver
  useEffect(() => {
  ...
  const handleBagCapacityChange = (newCapacity) => {
    ...
    patchUser({
      driver: {
        ...(user?.driver || {}),
              bag_capacity_liters: newCapacity,
      },
    });
  };
  ...
  return (
    ...
    {/* Sección Driver — solo visible para conductores */}
    {isDriver && (
      <DriverSection
      token={auth.token}
